...
import com.github.jelmerk.hnswlib.core.{Index => JIndex}
...
/**
  ...
  */
@SerialVersionUID(1L)
private[scala] class ScalaIndexAdapter[TId, TVector, TItem <: Item[TId, TVector], TDistance](val delegate: JIndex[TId, TVector, TItem, TDistance])
  ...
  override def findNearest(vector: TVector, k: Int): Seq[core.SearchResult[TItem, TDistance]] =
    new UnsafeImmutableJListWrapper(delegate.findNearest(vector, k))

  override def findNeighbors(id: TId, k: Int): Seq[core.SearchResult[TItem, TDistance]] =
